...
export default async function handler(req, res) {
  ...
  const { type, query, chain, targetTitle } = req.body;

  try {
    let prompt = "";

    if (type === 'initial') {
      prompt = `L'utente cerca: "${query}". Genera esattamente 20 titoli condensati e rilevanti correlati al tema. Per ogni titolo, includi facoltativamente una lista di sottotitoli correlati.
Restituisci ESCLUSIVAMENTE un JSON con questo formato:
{
  "titles": [
    { "title": "Titolo 1", "subtitles": ["Sotto 1", "Sotto 2"] },
    ...
  ]
}`;
    } else if (type === 'expand') {
...
Restituisci ESCLUSIVAMENTE un JSON con questo formato:
{ "titles": ["Nuovo Titolo 1", "Nuovo Titolo 2", ...] }`;
